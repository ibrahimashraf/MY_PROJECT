...
*   [ ] **3.2: Universal FIPS 140-3 Hardware Tablet Attestation (`pkg/onboarding/onboarding_engine.go`, `field_app/lib/workpackages/`)**:
    ...
    *   🚀 **D3.2 progress (2026-09-08, big-pickle `85591ca` + `a318946`, server policy only)**: `AttestationClaim` (SECURE_ENCLAVE/STRONGBOX/SOFTWARE/NONE, opaque blob, biometric flag) + `VerifyClaim` policy (permissive default, opt-in RequireHardware/RequireBiometricBinding, unknown origins fail closed) wired into `ProcessDeviceEnrollment`; posture recorded on `DeviceTrustRecord`. Receipt side: `VerifyOfflineReceiptWithPolicy` delegates to existing crypto verification then enforces recorded posture (zero policy = legacy behavior). Gates: vet CLEAN, full `go test` PASS, race PASS on `pkg/onboarding`. Open: real App Attest / Play Integrity verification (needs hardware + Dart SDK work, not CI-provable).
