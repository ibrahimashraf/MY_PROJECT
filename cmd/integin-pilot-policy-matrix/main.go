...
func main() {
	if len(os.Args) != 2 {
		log.Fatal("usage: integin-pilot-policy-matrix <seed|exercise|policy-seed|policy-exercise>")
	}
	switch os.Args[1] {
	case "seed":
		if err := seed(); err != nil {
	...
	case "policy-seed":
		if err := policySeed(); err != nil {
			log.Fatal(err)
		}
	case "policy-exercise":
		if err := policyExercise(); err != nil {
			log.Fatal(err)
		}
	default:
		log.Fatal("usage: integin-pilot-policy-matrix <seed|exercise|policy-seed|policy-exercise>")
...
func seed() error {
	...
	//nolint:gosec // PrivateKey in fixture for test matrix; file written with 0600
