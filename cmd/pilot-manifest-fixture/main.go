...
func main() {
	...
	fmt.Printf("INSERT INTO work_package_assignment (tenant_id,organization_id,inspection_id,device_id,package_id,package_version,authority_epoch,expires_at,assigned_at) VALUES (%s,%s,'pilot-manifest-demo-inspection',%s,%s,1,1,%s,now()) ON CONFLICT (tenant_id,organization_id,inspection_id) DO NOTHING;\n", q(f.TenantID), q(f.OrganizationID), q(f.DeviceID), q(p.ID), q(expires.Format(time.RFC3339)))
	fmt.Printf("INSERT INTO work_package_assignment_context (tenant_id,organization_id,inspection_id,root_asset_id,inspection_type,procedure_version,scheduled_at,field_asset_ids) VALUES (%s,%s,'pilot-manifest-demo-inspection','pilot-manifest-demo-asset','pilot-manifest-demo','pilot-v1',now(),'{\"condition\":[\"pilot-manifest-demo-asset\"]}'::jsonb) ON CONFLICT (tenant_id,organization_id,inspection_id) DO NOTHING;\nCOMMIT;\n", q(f.TenantID), q(f.OrganizationID))
