...
$pilotRoot = 'C:\MY PROJECT\operations\pilot'
...
try {
  $javaArgumentString = '-Dkc.config.built=true -jar "{0}" start-dev --http-host=127.0.0.1 --http-port=18180 --http-management-host=127.0.0.1 --http-management-port=19090 --health-enabled=true' -f $runner
  $process = Start-Process -FilePath $java -ArgumentList $javaArgumentString -WorkingDirectory $keycloakHome -RedirectStandardOutput $stdoutPath -RedirectStandardError $stderrPath -PassThru
} finally {
