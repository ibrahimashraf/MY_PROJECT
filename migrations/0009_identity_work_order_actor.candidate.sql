...
BEGIN;
ALTER TABLE identity_membership ADD COLUMN actor_id TEXT;
ALTER TABLE identity_membership ADD COLUMN work_order_role TEXT CHECK (work_order_role IN ('inspector', 'manager', 'reviewer', 'administrator'));
-- Backfill is deliberately omitted: existing memberships require explicit local actor and role assignment.
ALTER TABLE identity_membership ADD CONSTRAINT identity_membership_actor_role_required CHECK ((actor_id IS NULL AND work_order_role IS NULL) OR (actor_id IS NOT NULL AND work_order_role IS NOT NULL));
