...
CREATE TABLE certificate_renewal_attempt (
    id TEXT PRIMARY KEY,
    ...
    previous_status TEXT NOT NULL CHECK (status IN ('DRAFT', 'PENDING_REVIEW', 'APPROVED', 'SIGNED', 'ISSUED', 'EXPIRED', 'REVOKED', 'SUPERSEDED')),
...
CREATE INDEX certificate_renewal_attempt_cert_idx
...
CREATE INDEX certificate_renewal_attempt_authority_idx
...
ALTER TABLE certificate_record
    ADD COLUMN renewal_count BIGINT NOT NULL DEFAULT 0 CHECK (renewal_count >= 0);

ALTER TABLE certificate_record
    ADD COLUMN last_renewal_attempt TIMESTAMPTZ;

ALTER TABLE certificate_record
    ADD COLUMN renewal_authority_token BYTEA;
