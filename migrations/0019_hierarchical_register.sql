...
CREATE INDEX IF NOT EXISTS location_area_branch_idx ON location_area (tenant_id, organization_id, branch_id);
CREATE INDEX IF NOT EXISTS location_zone_area_idx ON location_zone (tenant_id, organization_id, area_id);
...
CREATE TABLE IF NOT EXISTS equipment_type_field (
    id TEXT PRIMARY KEY,
    tenant_id TEXT NOT NULL,
    organization_id TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS equipment_type_field_type_idx ON equipment_type_field (tenant_id, organization_id, equipment_type_id);

-- Asset extended with hierarchy + type + custom values
ALTER TABLE asset_registry ADD COLUMN IF NOT EXISTS branch_id TEXT REFERENCES location_branch(id) ON DELETE SET NULL;
ALTER TABLE asset_registry ADD COLUMN IF NOT EXISTS area_id TEXT REFERENCES location_area(id) ON DELETE SET NULL;
ALTER TABLE asset_registry ADD COLUMN IF NOT EXISTS zone_id TEXT REFERENCES location_zone(id) ON DELETE SET NULL;
ALTER TABLE asset_registry ADD COLUMN IF NOT EXISTS equipment_type_id TEXT REFERENCES equipment_type(id) ON DELETE SET NULL;
ALTER TABLE asset_registry ADD COLUMN IF NOT EXISTS custom_fields JSONB DEFAULT '{}'::jsonb;

CREATE INDEX IF NOT EXISTS asset_registry_branch_idx ON asset_registry (tenant_id, organization_id, branch_id) WHERE branch_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS asset_registry_area_idx ON asset_registry (tenant_id, organization_id, area_id) WHERE area_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS asset_registry_zone_idx ON asset_registry (tenant_id, organization_id, zone_id) WHERE zone_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS asset_registry_type_idx ON asset_registry (tenant_id, organization_id, equipment_type_id) WHERE equipment_type_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS asset_registry_custom_gin ON asset_registry USING GIN (custom_fields) WHERE custom_fields IS NOT NULL AND custom_fields != '{}'::jsonb;
