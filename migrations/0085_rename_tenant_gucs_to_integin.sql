-- Rename tenant RLS session variables integin.* -> integin.* across all policies.
--
-- Application code (pgtx.BeginScope) dual-sets both the integin.* and the
-- legacy integin.* names in one statement, so this rewrite is safe to apply at
-- any point after the code deploy: pre-migration policies (integin.*) and
-- post-migration policies (integin.*) are both satisfied during rollout.
-- Afterwards only integin.* is read by any code path.
--
-- Policies are rebuilt from pg_catalog (name, command, roles,
-- permissive/restrictive, USING/WITH CHECK) with only the GUC names
-- substituted; nothing else about any policy changes. Historical migration
-- files are left frozen.
...
BEGIN
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION '0085: integin.* tenant GUC references remain in RLS policies after rewrite';
  END IF;
END $$;
