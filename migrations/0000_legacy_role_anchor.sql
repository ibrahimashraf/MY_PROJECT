...
-- integin_runtime, and GRANT statements name it. History is frozen, so those
...
-- integin_runtime; nothing may log in as integin_runtime.

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'integin_runtime') THEN
    CREATE ROLE integin_runtime WITH NOLOGIN;
  END IF;
END $$;
