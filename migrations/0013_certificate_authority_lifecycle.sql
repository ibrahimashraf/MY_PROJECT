...
CREATE UNIQUE INDEX IF NOT EXISTS certificate_record_one_active_revision ON certificate_record (tenant_id, organization_id, inspection_id, inspection_revision) WHERE status IN ('DRAFT', 'PENDING_REVIEW', 'APPROVED', 'SIGNED', 'ISSUED', 'EXPIRED');
...
CREATE INDEX IF NOT EXISTS certificate_record_scope_status_idx ON certificate_record (tenant_id, organization_id, status, created_at DESC);
CREATE INDEX IF NOT EXISTS certificate_audit_event_certificate_idx ON certificate_audit_event (tenant_id, organization_id, certificate_id, occurred_at);
