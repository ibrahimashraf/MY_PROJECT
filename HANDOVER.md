...
## 2026-09-24 Owner Decisions + Live Revocation (UNCOMMITTED)
- **Decision 1 (owner):** client-side outbox re-signing is **interim only**; the server-side reconcile contract is the next build. The shipped migration is NOT reverted — it is the only thing preventing inspector work loss today; reverting re-opens the `pm clear`-or-lose-work bug.
- **Decision 2 (owner):** no change to `secretStr` alignment — mint and validate already agree on the same value, so the change would be cosmetic. Skipped (reuse ladder rung 1).
- **Correction to an earlier claim of mine:** revocation was NOT absent. `POST /v1/devices/{id}/revoke` → `deviceenrollhttp/handler.go:501` persists REVOKED + `RevokeAuthority` for every authority. My "no live revocation wiring" came from grepping only 3 packages and missing `deviceenrollhttp`.
- **Real gap (now fixed):** revocation was durable but not *live* — `Processor.devices` and `AuthorityRegistry` are boot snapshots, and `AuthorityPackage` has no revocation field, so a revoked device kept syncing until restart. Fix: `AuthorityRegistry.Revoke/RevokeDeviceAuthorities` tombstones (survives identical-package replay, clears on genuine reissue), `Processor.RevokeDevice` drops the device so `Submit` fails closed, `deviceenrollhttp.Config.OnDeviceRevoked` callback fires after the DB write, wired in `run.go`. Boot now logs a WARNING when `INTEGIN_DB_URL` is absent and revocation is therefore unreachable.
- Upload-token gate hardened in the same pass: token claims are now checked against the live registry (`device`, `epoch`, unexpired) — a valid HMAC alone no longer suffices.
- Verification: `gofmt -l` clean, `go vet ./...` clean, `go test -count=1 ./...` no failures, 4 new tests (registry tombstone/reissue, runtime-authority revocation, revoked-device sync rejection, cache-notification callback + no-notify-on-reject). **Race detector: green.** cgo toolchain lives at `D:\LEIMS-TOOLS\msys64\mingw64\bin` (gcc 16.1.0) — run as `$env:PATH='D:\LEIMS-TOOLS\msys64\mingw64\bin;'+$env:PATH; $env:CGO_ENABLED='1'; go test -race -count=1 ./...`. Live: host 8081 boots (9 authorities, no revocation warning), provision → 170-char token → TUS create/append/complete → key, forged token → 401.
- Pre-existing flake fixed: `internal/idempotency/middleware_test.go` asserted a 5ms wall-clock SLA that the race detector intermittently exceeds under full-suite load (failed 1 of 3 full race runs; passes alone and with `-count=5`). The real invariant (`executed == 0`) is still asserted unconditionally; the SLA check is now skipped only when the `race` build tag is set (`race_on_test.go` / `race_off_test.go`). Full race suite green afterwards.
- Not done: live admin-session revoke E2E — **now DONE and green** (see below).
- **Live admin-session revoke E2E (2026-09-25, ALL PASS, no restart between revoke and assertions):** stub OIDC mints a real RS256 JWT for a seeded `pilot/revoke-admin` subject → admin membership + `admin` capability in `identity_membership_capability` (bootstrap via direct SQL, since the grant API itself needs an already-resolvable principal) → `POST /v1/devices/{id}/revoke` = **200, `status=REVOKED`, `authority_epoch` 1→2** → same upload token now **401** (registry tombstone) → same device's sync **403 REJECTED "authority package is not registered"** → DB shows `device_registry.state=REVOKED` + reason, `authority_package.revoked_at` set. Runner: `C:\Users\hima3\AppData\Local\Temp\opencode\revoke_run.ps1` (starts host server, runs `revoke_e2e.py`, stops it).
  - Which layer bites: the **registry tombstone fires first** at the HTTP boundary; the processor's device drop is the fallback (unit-verified in `TestRevokedDeviceCannotSyncWithoutRestart`).
  - Schema gotchas found live: `identity_membership.work_order_role` is CHECK-constrained to `inspector|reviewer|manager|administrator`, so `hasTenantAdmin`'s `work_order_role == "Tenant Administrator"` comparison could never match — the role branch was **unreachable in production** and only the `admin` capability path could pass the gate. **Now fixed** (see tenant-admin branch fix below). Also `identity_membership` PK is a generated column, so `ON CONFLICT DO NOTHING` never dedupes: repeated grants create extra rows and make resolution **ambiguous** (403 `authorization_failed`). Harness must delete-then-insert exactly one membership.
  - Residue: the E2E is now **self-cleaning** in FK order (`sync_receipt` → `sync_held_transaction` → `sync_device_state` → `authority_package` → `device_registry`); DB confirmed back to 8 TRUSTED / 0 revoked.
- **Tenant-admin branch fix (submodule commit `2200378`, 2026-09-25):** `hasTenantAdmin` compared the work-order role against `"Tenant Administrator"`, a spelling the `work_order_role` CHECK constraint forbids, so `POST /v1/devices/{id}/revoke` and the enrollment-approval endpoint were unreachable through the sanctioned grant vocabulary. Changed the comparison to `"administrator"` — the token the column can actually hold and the same one `identity.GrantMembership` accepts. Chosen over deleting the dead branch so the documented role-based path is live.
  - **Privilege note (needs owner awareness):** this makes tenant-admin operations authorizable via the `administrator` work-order role. It grants no new powers to `inspector`/`reviewer`/`manager` (negative cases asserted), but it is a privilege-relevant widening of who can reach approve/revoke.
  - Test helper `adminByRoleOrg` now injects the real token instead of the fictional one, so the suite can no longer pass by bypassing the projection.
  - New `TestTenantAdminAuthorizationFollowsRealProjection` builds a real `identity.Membership` → `ProjectMembership` → `hasTenantAdmin` (not a hand-built `org.Roles`) and asserts both positive paths plus two negatives. **Proven to catch the bug:** reverting the one-liner fails it with "grantable administrator role must authorize tenant admin operations".
  - Live proof with the `admin` capability deliberately **not** granted — admin held only by the grantable `administrator` role, capabilities `{workorder.add_evidence_reference}`: revoke **200 REVOKED** (epoch 1→2) → same upload token **401** → same device sync **403** → DB `REVOKED` → self-cleanup. `FINAL: ALL PASS`.
  - Gates after fix: `gofmt -l` clean, `go vet ./...` clean, `go test -count=1 ./...` green, `go test -race -count=1 ./...` green (no data races).
  - Commit contains **only** the two `deviceenrollhttp` files, deliberately: the other lane's in-flight `MintUploadToken` signature change (added tenant + session) required updating my call sites in `internal/server/tus_auth_test.go` and `internal/localprovision/upload_token_test.go`, and committing those without their `upload_token.go` refactor would break a standalone build. Those two files remain uncommitted and must land with that refactor.
