...
Prove that the applied pilot database returns only canonical inspection records belonging to the active tenant and organization when queried as the non-owner `integin_pilot_runtime` role. The proof is read-isolation evidence only; it must not alter any privilege, RLS policy, migration, server startup, certificate, or external-authority path.

## Verified preconditions

| Precondition | Observation | Consequence |
|---|---|---|
| Runtime role | `integin_pilot_runtime` is neither superuser nor `BYPASSRLS` | A session assuming this role is suitable for an RLS proof. |
...
| Role assumption | `integin_pilot_owner` is a member of `integin_pilot_runtime` | A transaction can use `SET LOCAL ROLE integin_pilot_runtime` without exposing a role credential. |
