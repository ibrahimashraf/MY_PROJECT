...
The immediate priority is to finish the current Go/Flutter platform, not to introduce a rewrite. The controlled local Flutter provisioning path, a real Flutter-to-Go signed sync/evidence acceptance run, PostgreSQL isolated restore drill, RustFS archive/manifest restore drill, and documented operator recovery procedure are now complete. The next priority is to replace the local bridge with a production enrollment design and establish the governance backlog in `PLATFORM_GOVERNANCE_BACKLOG.md`. [1] [3]
