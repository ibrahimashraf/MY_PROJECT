...
class SyncClient {
  ...
  /// Re-submits a previously applied transaction for server idempotency proof.
  Future<SyncOutcome> replay(OutboxEntry entry) async {
    final response = await transport.submit(entry.mutation);
    await store.mark(entry, _stateFor(response.outcome),
        error: response.reason);
