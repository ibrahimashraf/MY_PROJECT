...
void main() {
  group('CryptographicMerkleHashChain', () {
    ...
    test('verifies untampered chain and rejects modified entries', () {
      final chain = CryptographicMerkleHashChain();
      final context = const TenantContext(
        tenantId: 'tenant_1',
        organizationId: 'org_1',
      ...
      final entries = [
        OutboxEntry(mutation: m1, chainHash: h1),
        OutboxEntry(mutation: m2, chainHash: h2),
      ];

      final verifier = CryptographicMerkleHashChain();
      expect(verifier.verifyChain(entries), isTrue);

      // Tampering test: tamper with entry 1's chainHash
      final tamperedEntries = [
        OutboxEntry(mutation: m1, chainHash: 'TAMPERED_HASH'),
        OutboxEntry(mutation: m2, chainHash: h2),
      ];
      expect(verifier.verifyChain(tamperedEntries), isFalse);
    });
  });
}
