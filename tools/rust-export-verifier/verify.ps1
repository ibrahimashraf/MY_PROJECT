...
# tools/rust-export-verifier. Regenerates the golden fixture with the Go
...
try {
    ...
    Push-Location tools/rust-export-verifier
    try {
        cargo build --release
    } finally {
        Pop-Location
    ...
    & tools/rust-export-verifier/target/release/integin-export-verifier.exe `
        --manifest tools/rust-export-verifier/testdata/manifest.json `
        --public-key tools/rust-export-verifier/testdata/public_key.hex
