...
func warnIfUnconfigured(tsaConfigured bool, tusEnabled bool, oidcConfigured bool, evidenceMounted bool) {
	if !tsaConfigured {
	...
	if !evidenceMounted {
		log.Print("WARNING: evidence store is not configured — /evidence is unmounted, set INTEGIN_EVIDENCE_STORE to enable")
	}
}
